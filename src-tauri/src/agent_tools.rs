...
use crate::agent_service::{PendingAsks, hidden_cmd, async_output_with_timeout, async_execute_via_shell, default_shell, shell_args};
...
#[allow(dead_code)]
pub(crate) async fn tool_run_command(params: &serde_json::Value, session_id: i64) -> String {
    ...
    if needs_shell && !is_shell_program(&program) {
        ...
        return async_execute_via_shell(
            &command, cwd.as_deref(), timeout_secs, session_id,
        ).await;
    }
    ...
    if is_shell_program(&program) {
        ...
        return async_output_with_timeout(
            &program, &args, cwd.as_deref(), timeout_secs, session_id,
        ).await;
    }

    // Path 3: simple command — try direct, fall back to shell on spawn/wait error.
    // The fallback must NOT trigger on a successful command that happens to
    // print text starting with "Error..." (e.g. `echo "Error: foo"`). So we
    // distinguish spawn / wait failures from normal output using a sentinel
    // prefix set only on the failure paths in async_output_with_timeout.
    let result = async_output_with_timeout(
        &program, &args, cwd.as_deref(), timeout_secs, session_id,
    ).await;
    if !is_command_spawn_error(&result) {
        return result;
    }

    // Fallback to system shell
    async_execute_via_shell(&command, cwd.as_deref(), timeout_secs, session_id).await
}

/// True only if `result` is one of the known spawn / wait failure messages
/// produced by async_output_with_timeout — never a successful command whose
/// stdout happens to start with the word "Error".
#[allow(dead_code)]
fn is_command_spawn_error(result: &str) -> bool {
    result.starts_with("Error spawning command:")
        || result.starts_with("Error waiting for process:")
}

/// Streaming variant of `tool_run_command`. Emits exec_command_begin/delta/end
/// events to the frontend ToolCard (via `command_stream::async_run_streaming`),
/// then returns the same formatted string the LLM receives — so the prompt
/// format is unchanged from the pre-streaming path.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn tool_run_command_streaming(
    params: &serde_json::Value,
    session_id: i64,
    call_id: String,
    tool_id: String,
    cancel_registry: crate::command_stream::CancelRegistry,
    pid_registry: crate::command_stream::CommandPidRegistry,
    app: AppHandle,
    session_key: String,
) -> String {
    let command = params["command"].as_str().unwrap_or("").to_string();
    let cwd_str: Option<String> = params["path"].as_str().map(normalize_file_path);
    let cwd_disp: Option<String> = cwd_str.clone();
    let timeout_secs = params["timeout"].as_u64().unwrap_or(COMMAND_TIMEOUT_SECS);
    if command.is_empty() {
        return "Error: command is required".to_string();
    }
    ...
    // Emit Begin up front so the ToolCard transitions to "streaming" immediately
    emit_command_begin(&app, &session_key, &call_id, &tool_id, &command, cwd_disp.as_deref());

    // Build the (program, args) per the existing 3-path shell routing strategy.
    let (prog, prog_args): (String, Vec<String>) = if needs_shell && !is_shell_program(&program) {
        let shell = default_shell();
        let args = shell_args(shell, &command);
        (shell.to_string(), args)
    } else if is_shell_program(&program) {
        (program, args)
    } else {
        // Path 3: try direct first, fall back to system shell on spawn failure.
        let direct = crate::command_stream::async_run_streaming(
            &program, &args, cwd_str.as_deref(), timeout_secs, session_id,
            call_id.clone(), tool_id.clone(),
            cancel_registry.clone(), pid_registry.clone(),
            app.clone(), session_key.clone(),
        ).await;
        // Detect spawn failure by exit_code=None AND no captured output
        if direct.2.is_none() && direct.0.is_empty() && direct.1.starts_with("Error spawning") {
            // Fall through to shell
            let shell = default_shell();
            let args = shell_args(shell, &command);
            (shell.to_string(), args)
        } else {
            return format_command_result(&direct, timeout_secs);
        }
    };

    let out = crate::command_stream::async_run_streaming(
        &prog, &prog_args, cwd_str.as_deref(), timeout_secs, session_id,
        call_id, tool_id,
        cancel_registry, pid_registry,
        app, session_key,
    ).await;

    format_command_result(&out, timeout_secs)
}

fn format_command_result(
    out: &(String, String, Option<i32>, bool, bool, usize),
    timeout_secs: u64,
) -> String {
    let (stdout, stderr, exit_code, killed, truncated, total_len) = out;
    if *killed {
        let summary = if stdout.is_empty() && stderr.is_empty() {
            String::new()
        } else {
            format!("Partial output:\n{}{}", stdout, stderr)
        };
        return format!("Command timed out after {}s (killed)\n{}", timeout_secs, summary);
    }
    let exit = exit_code.unwrap_or(-1);
    let suffix = if *truncated {
        format!(" [truncated — {} total chars]", total_len)
    } else {
        String::new()
    };
    if stdout.is_empty() && !stderr.is_empty() {
        ...
        format!("Exit: {}{}\n{}\n{}", exit, suffix, stdout, stderr)
    } else {
        format!("Exit: {}{}\n{}", exit, suffix, stdout)
    }
}

fn emit_command_begin(
    app: &AppHandle,
    session_key: &str,
    call_id: &str,
    tool_id: &str,
    ...
    let _ = app.emit(
        session_key,
        crate::agent_service::StreamEvent::CommandBegin {
...
/// Streaming variant of `tool_run_background` — same protocol as
/// tool_run_command_streaming, but does not block waiting for completion:
/// the LLM gets the result so far when the call returns. The frontend
/// ToolCard continues to receive live deltas via the same channel.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn tool_run_background_streaming(
    params: &serde_json::Value,
    session_id: i64,
    call_id: String,
    tool_id: String,
    ...
    emit_command_begin(&app, &session_key, &call_id, &tool_id, command, Some(&path));
    ...
    // For run_background, we let it run; collect up to `wait_sec` of output
    // and return that as the LLM-facing result. The async task keeps running
    // and continues to emit deltas + eventually an End event.
    let (prog_for_run, args_for_run): (String, Vec<String>) = if has_shell_metacharacters(command) {
        let shell = default_shell();
        let args = shell_args(shell, command);
        (shell.to_string(), args)
    } else {
    ...
    // Snapshot call: runs the command with a wait_sec timeout, returns the partial output
    let snapshot = run_with_snapshot(
        &prog_for_run, &args_for_run, Some(&path), session_id,
        call_id.clone(), tool_id.clone(),
        cancel_registry.clone(), pid_registry.clone(),
        app.clone(), session_key.clone(), wait_sec,
    ).await;

    // Follow-up: re-run the same command with the full COMMAND_TIMEOUT_SECS to completion,
    // so the ToolCard continues to receive deltas and the End event eventually fires.
    let prog_bg = prog_for_run.clone();
    let args_bg = args_for_run.clone();
    let path_bg = path.clone();
    let app_bg = app.clone();
    let session_key_bg = session_key.clone();
    let cancel_reg_bg = cancel_registry.clone();
    let pid_reg_bg = pid_registry.clone();
    tokio::spawn(async move {
        let _ = crate::command_stream::async_run_streaming(
            &prog_bg, &args_bg, Some(&path_bg), COMMAND_TIMEOUT_SECS, session_id,
            crate::command_stream::generate_call_id(),
            String::new(), // no LLM card to associate for the follow-up tail
            cancel_reg_bg, pid_reg_bg,
            app_bg, session_key_bg,
        ).await;
    });

    format!(
        "Background task started (PID tracked). Snapshot after {}s:\n{}",
        wait_sec, snapshot
    )
}

/// Run a command and return a snapshot of its output after `wait_sec` seconds.
/// The command itself is NOT killed when the snapshot is taken — it continues
/// to run in the background and the caller is expected to spawn a follow-up
/// `async_run_streaming` task that waits for full completion.
async fn run_with_snapshot(
    ...
    // Run the streaming variant with a `wait_sec` timeout — when it fires,
    // the function returns with whatever output has accumulated so far, marked
    // as killed=true. The follow-up task (spawned by the caller) re-runs the
    // same command and lets it complete naturally, re-emitting End.
    let (stdout, stderr, exit_code, _killed, _truncated, _total_len) =
        crate::command_stream::async_run_streaming(
            program, args, cwd, wait_sec, session_id,
            call_id, tool_id,
            cancel_registry, pid_registry,
            app, session_key,
        ).await;
    if let Some(code) = exit_code {
        ...
        format!("(still running after {}s)\n{}{}", wait_sec, stdout, stderr)
    }
}
...
pub(crate) async fn tool_run_background(params: &serde_json::Value, session_id: i64) -> String {
    ...
    // Temp dir for output capture (ULID-like naming via make_output_files)
    let (out_file, err_file) = crate::background_tasks::make_output_files(session_id, command);
    ...
    let (task_id, pid) = match try_spawn(&program, &args, &out_file, &err_file) {
        ...
        Err(_first_err) => {
            ...
            if !needs_shell && !is_shell_program(&program) {
                let shell_prog = default_shell();
                let shell_args = shell_args(shell_prog, command);
...
/// Wait for a background job to complete or match a ready signal.
/// Blocks until the process exits, a ready signal is detected, or timeout.
/// Returns structured JSON with status, output, and timing info.
pub(crate) async fn tool_wait_for_job(params: &serde_json::Value) -> String {
    let job_id = params["job_id"].as_u64().unwrap_or(0);
    let timeout_secs = params["timeout_secs"].as_u64().unwrap_or(60);
    let out_file_param = params["out_file"].as_str().unwrap_or("");

    // Look up task by job_id from registry
    let (pid, out_file, already_exited, _exit_code) = match crate::background_tasks::get_task(job_id) {
        Some(info) => info,
        None => {
            return format!("Error: job_id {} not found in registry", job_id);
        }
    };

    // Use provided out_file or looked-up one
    let out_file = if !out_file_param.is_empty() {
        out_file_param.to_string()
    } else {
        out_file
    };

    let path = std::path::PathBuf::from(&out_file);
    if !path.exists() {
        return format!("Error: output file not found: {}", out_file);
    }

    // If already exited, return immediately
    if already_exited {
        let final_output = std::fs::read_to_string(&path).unwrap_or_default();
        return format!(
            r#"{{"status":"exited","job_id":{},"matched_signal":null,"elapsed_ms":0,"output":"{}"}}"#,
            job_id,
    ...
    let mut matched_signal: Option<&str> = None;
    ...
    loop {
        ...
        if !crate::background_tasks::is_task_alive(pid) {
            status = "exited";
            break;
        }
        ...
        if current_size > last_size {
            ...
            use std::io::Seek;
            if file.seek(std::io::SeekFrom::Start(last_size)).is_err() {
                break;
            }
            ...
            let new_output = String::from_utf8_lossy(&buf).into_owned();

            // Check for ready signal
            if let Some(signal) = crate::background_tasks::detect_ready_signal(&new_output) {
                matched_signal = Some(signal);
                status = "ready";
                break;
            }

            // Check full output for signals
            let full_output = std::fs::read_to_string(&path).unwrap_or_default();
            if let Some(signal) = crate::background_tasks::detect_ready_signal(&full_output) {
                matched_signal = Some(signal);
                status = "ready";
                break;
            }
    ...
    format!(
        r#"{{"status":"{}","job_id":{},"matched_signal":{},"elapsed_ms":{},"output":"{}"}}"#,
        status,
        job_id,
        matched_signal.map(|s| format!("\"{}\"", s)).unwrap_or("null".to_string()),
        elapsed_ms,
        final_output.replace('"', "\\\"")
    )
}
...
pub(crate) async fn tool_job_output(params: &serde_json::Value) -> String {
    ...
    // Look up task info by job_id from registry
    let (pid, out_file) = match crate::background_tasks::get_task(job_id) {
        Some((p, f, _, _)) => (p, f),
        None => {
            return format!("Error: job_id {} not found in registry (use out_file to read directly)", job_id);
        }
    };
    ...
    let status = crate::background_tasks::is_task_alive(pid);
...
pub(crate) async fn tool_list_jobs(_params: &serde_json::Value, session_id: i64) -> String {
    let tasks = crate::background_tasks::list_tasks(session_id);
    if tasks.is_empty() {
        return "No background tasks for this session.".to_string();
    }
    let lines: Vec<String> = tasks.iter().map(|t| {
        let status = if t.running { "running" } else { "exited" };
        let exit_str = t.exit_code.map(|c| format!(" (exit {})", c)).unwrap_or_default();
        format!("[{}] {} - {} (pid: {}){}", t.id, t.command, status, t.pid, exit_str)
    }).collect();
    lines.join("\n")
}

pub(crate) async fn tool_spawn_process(params: &serde_json::Value, session_id: i64) -> String {
    let command = params["command"].as_str().unwrap_or("");
    ...
    tokio::task::spawn_blocking(move || {
        ...
        match cmd.spawn() {
            Ok(child) => {
                ...
                // Keep child alive so PM can kill it later via PID
...
pub(crate) async fn tool_kill_process(params: &serde_json::Value) -> String {
    ...
    tokio::task::spawn_blocking(move || {
        match crate::process_manager::kill_tree(pid) {
...
/// Write `data` to `path` atomically: write to a sibling temp file in the same
/// directory, fsync, then rename. Prevents partial-write corruption if the
...
pub(crate) fn atomic_write(path: &std::path::Path, data: &[u8]) -> std::io::Result<()> {
    ...
    let tmp = parent.join(format!(".{}.minimax-tmp", file_name));
    {
    ...
    for entry in entries.filter_map(|e| e.ok()) {
        ...
        if file_path.is_dir() {
            ...
            if let Ok(content) = std::fs::read_to_string(&file_path) {
                ...
                if new_content != content && atomic_write(&file_path, new_content.as_bytes()).is_ok() {
                    *count += 1;
                }
            }
        }
...
pub(crate) fn repair_truncated_json(json_str: &str) -> String {
    ...
    // If still not valid JSON, try to wrap in {} and validate again. Log when
    // repair ultimately fails so the upstream caller can decide to retry/abort
    // instead of silently feeding invalid input to the tool dispatcher.
    if serde_json::from_str::<serde_json::Value>(&s).is_err() {
        if !s.starts_with('{') {
            ...
            if serde_json::from_str::<serde_json::Value>(&wrapped).is_ok() {
                ...
                eprintln!(
                    "[repair_truncated_json] Repair failed; input did not become valid JSON. \
                     Original len={}, repaired len={}, head={:?}",
                    json_str.len(),
                    s.len(),
            ...
            eprintln!(
                "[repair_truncated_json] Repair failed; already an object but still invalid. \
                 Original len={}, repaired len={}, head={:?}",
                json_str.len(),
                s.len(),
                &s.chars().take(80).collect::<String>()
