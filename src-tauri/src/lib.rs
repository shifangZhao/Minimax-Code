...
use agent_service::{AgentService, StreamEvent};
...
use std::sync::{Arc, Mutex, MutexGuard, Mutex as StdMutex};
...
struct AppState {
    ...
    // Streaming-command protocol: per-call_id cancel + per-(session, call_id) pid tracking.
    command_cancel_registry: command_stream::CancelRegistry,
    command_pid_registry: command_stream::CommandPidRegistry,
    // Per-session concurrency control: ensures only one agent loop per session.
    runner: Arc<runner::RunnerRegistry>,
}
...
fn ctrlc_handler() {
    // Best-effort: kill all managed child processes on SIGTERM/SIGINT/Ctrl+C.
    // On Windows this uses SetConsoleCtrlHandler; on Unix, signal hooks.
    // We don't use the ctrlc crate to avoid adding a dependency — just spawn
    // a thread that blocks on stdin EOF (which fires on window close).
    std::thread::spawn(|| {
        // Block until stdin closes (window close) or process exits
        let mut buf = [0u8; 1];
        let _ = std::io::stdin().read(&mut buf);
        eprintln!("[exit] stdin closed, killing all managed processes");
        process_manager::kill_all();
    });
}

#[tauri::command]
fn list_managed_processes() -> Result<Vec<process_manager::ManagedProcess>, String> {
    Ok(process_manager::list_all())
}

#[tauri::command]
fn kill_managed_process(pid: u32) -> Result<(), String> {
    process_manager::kill_tree(pid)
}
...
fn get_process_stats() -> Result<serde_json::Value, String> {
    let processes = process_manager::list_all();
    let running = processes.iter().filter(|p| p.running).count();
    let total = processes.len();
    let total_memory = process_manager::total_memory_bytes();
    Ok(serde_json::json!({
        "running": running,
        "total": total,
...
fn get_group_chats(state: State<AppState>, mode: Option<String>) -> Result<Vec<GroupChat>, String> {
    ...
    let query = if mode.is_some() {
        "SELECT id, name, mode, workspace, created_at FROM group_chat WHERE mode = ?1 ORDER BY created_at DESC"
    } else {
        "SELECT id, name, mode, workspace, created_at FROM group_chat ORDER BY created_at DESC"
    };
...
fn get_agent_sessions(state: State<AppState>, group_chat_id: i64, agent_type: String) -> Result<Vec<AgentSession>, String> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, group_chat_id, agent_type, worktree_path, last_token_usage, created_at FROM agent_session WHERE group_chat_id = ?1 AND agent_type = ?2"
    ).map_err(|e| e.to_string())?;
...
#[tauri::command]
fn get_user_message_indices(state: State<AppState>, session_id: i64) -> Result<Vec<i64>, String> {
    let conn = state.db.lock().map_err(|e| e.to_string())?;
    // Exclude tool_result messages (stored as role='user' with tool_result parts)
    let mut stmt = conn.prepare(
        "SELECT DISTINCT cm.id FROM chat_message cm
         LEFT JOIN message_part mp ON mp.message_id = cm.id AND mp.part_type = 'tool_result'
         WHERE cm.session_id = ?1 AND cm.role = 'user' AND mp.id IS NULL
         ORDER BY cm.id ASC"
    ).map_err(|e| e.to_string())?;
    let ids: Vec<i64> = stmt.query_map(rusqlite::params![session_id], |row| row.get(0))
        .map_err(|e| e.to_string())?
        .filter_map(|r| r.ok())
        .collect();
    Ok(ids)
...
fn set_minimax_api_key(state: State<AppState>, api_key: String) -> Result<(), String> {
    // Reject if the frontend accidentally sends the already-masked placeholder
    // (e.g. user pastes the value they see in the settings UI). Storing the
    // masked form would break all subsequent API calls.
    if is_masked_key(&api_key) {
        return Err("Refusing to save a masked key; paste the full key".to_string());
    }
    let conn = state.db.lock().map_err(|e| e.to_string())?;
...
fn mask_api_key(key: &str) -> String {
    ...
    // Need at least head(2) + "****"(4) + tail(2) = 8 hidden chars, so anything
    // shorter than 8 characters cannot be safely masked with head/tail without
    // leaking the entire value. Return a fully redacted placeholder instead.
    if len < 8 {
        return "****".to_string();
    }
...
fn is_masked_key(s: &str) -> bool {
    // A properly masked key is exactly "head(4)****tail(4)" = 12 chars; the old
    // <=16 byte check let through short strings that happened to contain
    // "****" (e.g. a user-typed placeholder), causing set_minimax_api_key to
    // accept a masked value as a real key. Be strict: the input must be a
    // 12-character string of 4 alphanumerics + "****" + 4 alphanumerics.
    s.chars().count() == 12 && {
...
#[tauri::command]
async fn set_group_chat_workspace(state: State<'_, AppState>, group_chat_id: i64, workspace: String) -> Result<(), String> {
    {
        let conn = state.db.lock().map_err(|e| e.to_string())?;
        conn.execute(
            "UPDATE group_chat SET workspace = ?1 WHERE id = ?2",
            rusqlite::params![workspace, group_chat_id],
        ).map_err(|e| e.to_string())?;
    }
    state.skill_service.load_project_skills(&workspace).await;
    eprintln!("[set_group_chat_workspace] Loaded project skills for: {}", workspace);
    Ok(())
}

#[tauri::command]
fn get_group_chat_workspace(state: State<AppState>, group_chat_id: i64) -> Result<Option<String>, String> {
    let conn = state.db.lock().map_err(|e| e.to_string())?;
    let result: Option<String> = conn.query_row(
        "SELECT workspace FROM group_chat WHERE id = ?1",
        [group_chat_id],
        |row| row.get(0),
    ).ok();
    Ok(result)
...
fn list_custom_configs(state: State<AppState>) -> Result<Vec<CustomModelConfig>, String> {
    ...
    let rows = stmt.query_map([], |row| {
        ...
        Ok(CustomModelConfig {
            ...
            api_key: mask_api_key(&key), model: row.get(4)?,
...
#[tauri::command]
fn run_command(state: State<'_, AppState>, command: String, cwd: Option<String>) -> Result<CommandOutput, String> {
    // Permission gate: this tauri command runs arbitrary shell, so a malicious
    // frontend could otherwise call it directly and bypass the per-tool checks
    // in agent_tools. Enforce the same chain/command rules the agent uses.
    let mode = state.permission_service.lock()
        .map(|s| s.mode)
        .unwrap_or(PermissionMode::Normal);
    if mode != PermissionMode::Full {
        if let Some(Err(reason)) = permission::check_command_chain(&command) {
            return Err(format!("Blocked by permission policy: {}", reason));
        }
        // check_command_chain returns Some(Ok(())) for safe, None for needs-confirm.
        // Only auto-allow when we get a positive safe result. Otherwise, in
        // Normal mode we'd still want a confirm — but tauri commands are
        // fire-and-forget from the UI, so block anything that isn't auto-safe.
        if permission::check_command_chain(&command).is_none() {
            return Err("Command requires confirmation; use the agent's run_command tool instead".to_string());
        }
    }

    let (cmd, args) = if cfg!(windows) {
        let shell = crate::agent_service::default_shell();
        let args = crate::agent_service::shell_args(shell, &command);
        (shell, args)
    } else {
...
fn spawn_process(command: String, cwd: Option<String>) -> Result<i32, String> {
    ...
    let mut process = hidden_cmd(shell);
    if let Some(dir) = cwd {
        process.current_dir(dir);
    }
    for arg in &args {
    ...
    process_manager::register_process(
        pid,
        process_manager::ProcessType::Command,
        &command,
        None,
        None,
        None,
    );
    // Keep child alive so PM can kill it later
...
fn kill_process(pid: i32) -> Result<(), String> {
    process_manager::kill_tree(pid as u32)
}
...
#[tauri::command]
fn remove_bg_task(task_id: u64) -> Result<(), String> {
    background_tasks::remove_task(task_id);
    Ok(())
}
...
async fn agent_chat_stream(
    ...
    // If workspace not provided by frontend, resolve from group_chat
    let workspace = if workspace.is_some() {
        workspace
    } else {
        let conn = state.db.lock().map_err(|e| e.to_string())?;
        conn.query_row(
            "SELECT gc.workspace FROM group_chat gc
             JOIN agent_session as2 ON as2.group_chat_id = gc.id
             WHERE as2.id = ?1",
            [session_id],
    ...
    // Read context from DB and append the new user message
    // context_messages is stored as API-format JSON (role + structured content),
    // so we read it as serde_json::Value to preserve tool_use/tool_result blocks.
    let messages: Vec<serde_json::Value> = {
        let conn = state.db.lock().map_err(|e| e.to_string())?;
        let ctx_json: String = conn.query_row(
            "SELECT context_messages FROM agent_session WHERE id = ?1",
            rusqlite::params![session_id],
            |row| row.get(0),
        ).unwrap_or_else(|_| "[]".to_string());

        let mut ctx: Vec<serde_json::Value> = serde_json::from_str(&ctx_json)
            .unwrap_or_default();

        // Build user content with optional attachments
        let user_content = if let Some(ref att) = attachments {
            if !att.is_empty() {
                format!("{}\n\n[附件: {}]", new_message, att)
            } else {
                new_message.clone()
    ...
    let (api_key, api_url, messages_path, model, context_window, provider) = {
        ...
        if let Some((agent_provider, agent_model, agent_cw)) = agent_cfg {
            ...
            let cw = if agent_cw > 0 { agent_cw } else if agent_provider == "custom" {
                ...
                conn.query_row("SELECT context_window FROM app_config", [], |row| row.get::<_, i64>(0)).unwrap_or(1000000).max(0) as usize
            };
            ...
            match provider.as_str() {
                ...
                _ => {
                    let key: Option<String> = conn.query_row("SELECT api_key FROM minimax_api_key", [], |row| row.get(0)).ok();
                    let url: String = conn.query_row("SELECT api_url FROM app_config", [], |row| row.get(0)).unwrap_or_else(|_| DEFAULT_API_URL.to_string());
                    let m: String = conn.query_row("SELECT model FROM app_config", [], |row| row.get(0)).unwrap_or_else(|_| "MiniMax-M3".to_string());
                    let cw: i64 = conn.query_row("SELECT context_window FROM app_config", [], |row| row.get(0)).unwrap_or(1000000);
    ...
    // Auto-abort any existing stream for this session (graceful before creating new one)
    {
        ...
        process_manager::kill_session(session_id);
    ...
    // Acquire the runner for this session. If already running, wait briefly
    // for the abort to complete, then retry.
    if !state.runner.try_acquire(session_id) {
        // Old stream is still running — abort was signaled above, wait for it
        tokio::time::sleep(std::time::Duration::from_millis(200)).await;
        if !state.runner.try_acquire(session_id) {
            return Err("Session is busy — please wait".to_string());
        }
    }
    let runner = state.runner.clone();

    // Create agent service
    let service = AgentService::new(api_key, api_url, messages_path, model, context_window, provider, state.skill_service.clone(), state.mcp_service.clone(), state.db.clone(), state.lsp_manager.clone(), state.permission_service.clone(), state.pending_asks.clone(), state.command_cancel_registry.clone(), state.command_pid_registry.clone());
    ...
    let (cancel_tx, cancel_rx) = tokio::sync::watch::channel(false);
    {
        ...
        // If an old stream exists for this session, signal it to abort and drop
        // its sender. The task itself still lives in the runtime; we don't await
        // it here (would block the command response), but signaling ensures it
        // shuts down promptly on the next event-loop tick.
        if let Some(old_tx) = registry.remove(&session_id) {
    ...
    let _stream_task = tokio::spawn(async move {
        ...
        if let Err(e) = result {
            ...
            let _ = panic_app.emit(&panic_key, StreamEvent::Done);
        }
...
async fn abort_stream(state: State<'_, AppState>, app: AppHandle, session_id: i64) -> Result<(), String> {
    ...
    // 2. Kill running command processes via unified process manager
    process_manager::kill_session(session_id);

    // 3. Release the runner so the session can accept new commands
    state.runner.release(session_id);

    // 4. Signal cancel channel
    if let Ok(mut registry) = state.cancel_registry.lock() {
...
/// Abort a single in-flight command by its call_id (finer-grained than
/// abort_stream, which kills everything for a session). The matching
/// `async_run_streaming` task receives the cancel signal, kills the child
/// process, and emits `exec_command_end { killed: true }` so the frontend
/// ToolCard can transition to the killed state.
#[tauri::command]
async fn abort_command(
    state: State<'_, AppState>,
    call_id: String,
) -> Result<(), String> {
    if let Ok(mut reg) = state.command_cancel_registry.lock() {
        if let Some(tx) = reg.remove(&call_id) {
            let _ = tx.send(());
            eprintln!("[abort_command] Cancel signaled for call_id {}", call_id);
        }
    }
    Ok(())
}
...
pub(crate) async fn compact_messages(
    ...
    // Persist to DB: overwrite context_messages with compressed version.
    // chat_message/message_part are NOT touched — they remain for UI display.
...
async fn compact_session(state: State<'_, AppState>, session_id: i64) -> Result<String, String> {
    ...
    // Read context_messages directly — already in API format with full structured content
...
fn save_temp_file(name: String, data_url: String) -> Result<String, String> {
    // Reject path traversal and separators — name must be a bare filename.
...
fn rewind_conversation(state: State<AppState>, session_id: i64, message_id: i64) -> Result<String, String> {
    ...
    // 5. Rebuild context_messages from remaining messages
    // This ensures the LLM context matches the rewound conversation state.
    let remaining_messages: Vec<(i64, String, String)> = {
        let mut stmt = conn.prepare(
    ...
    // Load parts for remaining messages
    let mut context_messages: Vec<serde_json::Value> = Vec::new();
    for (msg_id, role, content) in &remaining_messages {
        // Skip system messages and empty tool_result messages
        if role == "system" { continue; }

        let parts: Vec<(String, String, Option<String>, Option<String>, Option<String>)> = {
            let mut stmt = conn.prepare(
                "SELECT part_type, content, tool_use_id, tool_name, tool_input FROM message_part WHERE message_id = ?1 ORDER BY part_order"
            ).map_err(|e| e.to_string())?;
        ...
        if parts.is_empty() {
            ...
            // Build structured content from parts
            let mut content_blocks: Vec<serde_json::Value> = Vec::new();
            for (part_type, part_content, tool_use_id, tool_name, tool_input) in &parts {
                match part_type.as_str() {
                    "text" => {
                        if !part_content.is_empty() {
                            content_blocks.push(serde_json::json!({"type": "text", "text": part_content}));
                        }
                    }
                    "thinking" => {
                        if !part_content.is_empty() {
                            content_blocks.push(serde_json::json!({"type": "thinking", "thinking": part_content}));
                        }
                    }
                    "tool_use" => {
                        let input: serde_json::Value = serde_json::from_str(tool_input.as_deref().unwrap_or("{}"))
                            .unwrap_or(serde_json::json!({}));
                        content_blocks.push(serde_json::json!({
                            "type": "tool_use",
                            "id": tool_use_id.as_deref().unwrap_or(""),
                            "name": tool_name.as_deref().unwrap_or(""),
                            "input": input
                        }));
                    }
                    "tool_result" => {
    ...
    // Update context_messages in agent_session
    let ctx_json = serde_json::to_string(&context_messages).unwrap_or_else(|_| "[]".to_string());
    conn.execute(
        "UPDATE agent_session SET context_messages = ?1 WHERE id = ?2",
        rusqlite::params![ctx_json, session_id]
    ).map_err(|e| e.to_string())?;

    drop(conn);

    // 6. Refresh LSP for all restored files
    if let Ok(mut mgr) = state.lsp_manager.lock() {
        if let Some(ref mut m) = *mgr {
...
pub fn run() {
    ...
    // ── Migrations ─────────────────────────────────────────────────────
    // Add last_token_usage column to agent_session if it doesn't exist
    conn.execute(
        "ALTER TABLE agent_session ADD COLUMN last_token_usage INTEGER NOT NULL DEFAULT 0",
        [],
    ).ok(); // Ignore error if column already exists

    // Add context_messages column — stores the LLM conversation context as JSON
    conn.execute(
        "ALTER TABLE agent_session ADD COLUMN context_messages TEXT NOT NULL DEFAULT '[]'",
        [],
    ).ok(); // Ignore error if column already exists

    // Add workspace column to group_chat — per-session working directory
    conn.execute(
        "ALTER TABLE group_chat ADD COLUMN workspace TEXT NOT NULL DEFAULT ''",
        [],
    ).ok(); // Ignore error if column already exists
    ...
    tauri::Builder::default()
        .manage(AppState {
            ...
            command_cancel_registry: Arc::new(StdMutex::new(HashMap::new())),
            runner: Arc::new(runner::RunnerRegistry::new()),
            command_pid_registry: Arc::new(StdMutex::new(HashMap::new())),
        })
        .setup(move |app| {
            // Start background task poller for frontend visualization
            background_tasks::start_poller(app.handle().clone());

            // Start periodic process cleanup (every 60s, clean exited processes older than 5min)
            tauri::async_runtime::spawn(async move {
                loop {
                    tokio::time::sleep(tokio::time::Duration::from_secs(60)).await;
                    process_manager::cleanup_exited(300);
                }
            });

            // Register exit cleanup using atexit-like mechanism
            // When the main thread exits, kill all managed child processes
            ctrlc_handler();
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            set_workspace,
            get_workspace,
            set_group_chat_workspace,
            get_group_chat_workspace,
            save_temp_file,
            read_file_base64,
            set_permission_mode,
            get_permission_mode,
