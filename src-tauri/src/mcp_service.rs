...
pub enum McpServerConfig {
    ...
    /// SSE transport: GET to url for SSE event stream, POST to discovered endpoint.
    #[serde(rename = "sse")]
    Sse {
        url: String,
        #[serde(default)]
...
struct SseTransport {
    ...
    get_url: String,
    /// POST URL for JSON-RPC requests (discovered from SSE endpoint event).
    post_url: Mutex<Option<String>>,
    client: HttpClient,
    headers: HashMap<String, String>,
    /// Queue of messages received from SSE stream.
    queue: Arc<Mutex<Vec<serde_json::Value>>>,
    /// Channels waiting for the next message.
    waiters: Arc<Mutex<Vec<std::sync::mpsc::Sender<serde_json::Value>>>>,
    alive: Arc<AtomicBool>,
}
...
impl Transport {
    async fn send_request(&self, request: &serde_json::Value, timeout_ms: u64) -> Result<serde_json::Value, String> {
        match self {
            ...
            Transport::Sse(t) => t.send_request(request, timeout_ms).await,
        }
    }
    ...
    async fn send_notification(&self, notification: &serde_json::Value) -> Result<(), String> {
        match self {
            ...
            Transport::Sse(t) => t.send_notification(notification).await,
        }
    }
    ...
    fn is_alive(&self) -> bool {
        match self {
            ...
            Transport::Sse(t) => t.alive.load(Ordering::SeqCst),
        }
    }
...
impl StdioTransport {
    fn spawn(command: &[String], env: &HashMap<String, String>, cwd: &str) -> Result<Self, String> {
        ...
        crate::process_manager::register_process(
            pid,
            crate::process_manager::ProcessType::Mcp,
            &command.join(" "),
            None,
            None,
            None,
...
impl Drop for StdioTransport {
    fn drop(&mut self) {
        ...
        let mut p = match self.process.lock_str() {
            Ok(p) => p,
            Err(_) => return,
        };
        // Best-effort graceful shutdown: ask the server to exit via JSON-RPC
        // notification, give it a moment to flush, then force kill if still alive.
        let shutdown = serde_json::json!({
            "jsonrpc": "2.0",
            "method": "shutdown",
        });
        if let Ok(mut w) = self.writer.lock_str() {
            let _ = w.write_all(serde_json::to_string(&shutdown).unwrap_or_default().as_bytes());
            let _ = w.write_all(b"\n");
            let _ = w.flush();
        }
        // Poll up to ~1s for graceful exit. We hold the lock briefly; the reader
        // thread holds no lock, so this won't deadlock.
        for _ in 0..20 {
            match p.try_wait() {
...
impl SseTransport {
    ...
    async fn send_request(&self, request: &serde_json::Value, _timeout_ms: u64) -> Result<serde_json::Value, String> {
        let post_url = {
            let guard = self.post_url.lock().unwrap();
            guard.clone().ok_or_else(|| "SSE endpoint not yet discovered — wait for connection".to_string())?
        };
    ...
    async fn send_notification(&self, notification: &serde_json::Value) -> Result<(), String> {
        let post_url = {
            ...
            match guard.clone() {
                Some(u) => u,
                None => {
                    // Queue notification to send once endpoint is discovered
                    let mut q = self.queue.lock().unwrap();
                    q.push(notification.clone());
                    return Ok(());
                }
            }
        };
    ...
    /// Start the SSE stream: GET to get_url, parse SSE events.
    /// When `endpoint` event arrives, store post_url and drain queued messages.
    /// Runs on the current tokio runtime (no nested runtime).
    fn start_streaming(&self) {
        let get_url = self.get_url.clone();
        let post_url = Arc::new(Mutex::new(self.post_url.lock().unwrap().clone()));
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let res = match client.get(&get_url)
                    ...
                    .await
                {
                    Ok(r) => r,
                    Err(e) => {
                        eprintln!("[mcp:sse] SSE connect to {} failed: {}", get_url, e);
                        alive.store(false, Ordering::SeqCst);
                        return;
                    }
                };
                if !res.status().is_success() {
                    eprintln!("[mcp:sse] SSE handshake {} → {}", get_url, res.status());
                    alive.store(false, Ordering::SeqCst);
                    return;
                }
                                ...
                                if trimmed.is_empty() {
                                    if !event_data.is_empty() {
                                        if event_type == "endpoint" {
                                            let endpoint_url = event_data.trim().to_string();
                                            {
                                                let mut pu = post_url.lock().unwrap();
                                                *pu = Some(endpoint_url.clone());
                                            }
                                            // Drain queued messages
                                            let msgs: Vec<serde_json::Value> = queue.lock().unwrap().drain(..).collect();
                                            for msg in msgs {
                                                let _ = client.post(&endpoint_url)
                                                    .header("Content-Type", "application/json")
                                                    .json(&msg)
                                                    .send()
                                                    .await;
                                    ...
                                    event_type = trimmed.trim_start_matches("event:").trim().to_string();
                                    in_data = true;
                                } else if in_data || trimmed.starts_with("data:") {
                                    if trimmed.starts_with("data:") {
                                        event_data.push_str(trimmed.trim_start_matches("data:"));
                                    } else {
                                        event_data.push_str(trimmed);
                                    }
                                    event_data.push('\n');
                                }
                            }
                ...
                // SSE stream ended — check if we're still alive (not closed)
...
impl McpService {
    ...
    async fn init_from_config(&self, config: &McpConfig, workspace: &str) -> Vec<McpServerStatus> {
        ...
        // Phase 1: under write lock, purge dead servers and decide what to connect.
        ...
        let to_connect: Vec<(String, McpServerConfig)> = {
            ...
            for (name, server_config) in &config.mcp_servers {
                ...
                to_connect.push((name.clone(), server_config.clone()));
            }
            to_connect
        };

        // Phase 2: connect each server without holding any lock.
        let mut connected: Vec<(String, Result<McpServer, String>)> = Vec::with_capacity(to_connect.len());
        for (name, server_config) in to_connect {
            let r = Self::connect_server(&name, &server_config, &cwd).await;
            connected.push((name, r));
        }

        // Phase 3: collect statuses for already-known/disabled, then insert new connections
    ...
    async fn connect_server(name: &str, config: &McpServerConfig, cwd: &str) -> Result<McpServer, String> {
        let (transport, timeout) = match config {
            ...
            McpServerConfig::Sse { url, headers, timeout, .. } => {
                let ms = timeout.unwrap_or(crate::MCP_HTTP_TIMEOUT_SECS * 1000);
                let t = SseTransport::new(url, headers, ms);
                t.start_streaming();
                (Transport::Sse(t), ms)
            }
        };
