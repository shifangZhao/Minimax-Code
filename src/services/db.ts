...
export interface UIMessage extends ChatMessage {
  /**
   * Original DB primary key as a number. Distinct from `id`, which may be
   * a synthetic string (e.g. `1558-t0`) when displayMessages splits one DB
   * row into multiple UI rows for thinking/text interleaving. Always use
   * `dbId` when calling backend commands that expect `i64`.
   */
  dbId?: number
  loading?: boolean
  cmdResult?: string
  ...
  thinking?: string
  tool_calls?: string
  /** Elapsed time in ms from user input to agent final response */
  duration_ms?: number
  raw_json?: string
}
...
export const db = {
  ...
  async setGroupChatWorkspace(groupChatId: number, workspace: string): Promise<void> {
    return invoke('set_group_chat_workspace', { groupChatId, workspace })
  },

  async getGroupChatWorkspace(groupChatId: number): Promise<string | null> {
    return invoke('get_group_chat_workspace', { groupChatId })
  },
  ...
  async getUserMessageIndices(sessionId: number): Promise<number[]> {
    return invoke('get_user_message_indices', { sessionId })
  },
