  ...
  <div class="agent-view" :class="agentType">
    <div class="agent-header">
      ...
      <div v-if="userMsgNav.count > 0" class="msg-nav">
        <button class="nav-btn" title="上一则用户消息" :disabled="userMsgNav.current <= 0" @click="navToUserMsg(-1)">
        ...
        <button class="nav-btn" title="下一则用户消息" :disabled="userMsgNav.current >= userMsgNav.count - 1" @click="navToUserMsg(1)">
          <svg width="11" height="11" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="header-actions">
        ...
        <span v-if="cacheUsage.ratio > 0" class="cache-badge" :title="`缓存命中: ${cacheUsage.hit} tokens / 未命中: ${cacheUsage.miss} tokens`">
          <svg width="12" height="12" viewBox="0 0 14 14" fill="currentColor">
            <path d="M7 1L3 8h3l-1 5 5-7H7l2-5z" />
          </svg>
      ...
      <div class="workspace-inline" :title="workspace || '输入工作目录路径'">
        ...
        <input
          ...
          @blur="saveWorkspaceEdit"
          @keydown.enter="saveWorkspaceEdit"
          @keydown.escape="cancelWorkspaceEdit"
          placeholder="输入工作目录路径..."
        />
        <button class="workspace-folder-btn" @click.stop="pickWorkspaceFolder" title="选择文件夹">📁</button>
      </div>
    </div>
    <ConfirmDialog
    ...
    <div class="messages" ref="messagesEl" @scroll="saveScrollPos" @click="handleLinkClick">
      <div v-if="hasMoreOlder" class="load-earlier-wrap">
        <button class="load-earlier-btn" :disabled="loadingMore" @click="handleLoadMore()">
      ...
      >
        ...
        <div class="content">
          <div v-if="msg.thinking && msg.role === 'assistant'" class="thinking-block">
            <div class="thinking-toggle" :class="{ collapsed: !isThinkingExpanded(msg.id) }" @click="toggleThinking(msg.id)">
              思考过程
              <span class="toggle-arrow"></span>
            </div>
            <div v-if="isThinkingExpanded(msg.id)" class="thinking-text" v-html="formatContent(msg.thinking)"></div>
          </div>
          ...
          <div class="msg-footer">
            ...
            <div class="duration" v-if="msg.duration_ms">{{ formatDuration(msg.duration_ms) }}</div>
          </div>
        </div>
        <div v-if="msg.role === 'user'" class="msg-hover-actions">
          <button class="hover-btn" title="重新生成" :disabled="msg.dbId == null" @click="retryMessageById(msg.dbId!)">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="M1 7a6 6 0 0111.6-2M13 7A6 6 0 011.4 9" />
              <path d="M1 1v5h5M13 13V8H8" />
            </svg>
          </button>
          <button class="hover-btn" title="回退到此" :disabled="msg.dbId == null" @click="rewindToMessage(msg.dbId!, msg.content)">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="M3 7h8M3 7l3-3M3 7l3 3" />
            </svg>
          </button>
      ...
      <template v-if="!currentStreaming.done">
        <template v-for="(seg, si) in streamingSegments" :key="si">
          ...
          <div v-else class="message tool">
            <div class="content">
              <div class="tool-msg">
                <ToolCard :toolInfo="{ name: seg.name || '', args: seg.args, result: seg.result, tool_id: seg.tool_id }" />
              </div>
            </div>
          </div>
        </template>
    ...
    />
    <ToastBar
      :visible="!!lastUndoError"
      :message="`撤销失败: ${lastUndoError || ''}`"
      @close="lastUndoError = null"
    />
    <AskDialog
      v-if="pendingAsk?.questions && pendingAsk.questions.length > 0"
    ...
    <div class="input-area" v-if="agentType === 'ace'">
      ...
      <div class="bg-popup" v-if="showBgPanel && bgTasks.size > 0">
        <div class="bg-popup-header">
          ...
          <button class="bg-popup-close" @click="showBgPanel = false">
            <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
      ...
      <textarea
        ...
        <div class="input-toolbar">
          ...
          <button v-show="!atBottom" class="scroll-bottom-btn" title="回到底部" @click="scrollToBottom(true)">
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="M7 3v8M3 7l4 4 4-4" />
            </svg>
          </button>
...
const {
  ...
  // Keep the undo list in sync with the streaming flow. Without this, the
  // watch on `messages.value.length` doesn't fire while tool_result
  // messages are being persisted (those rows are hidden in the UI so the
  // array length doesn't change), and the user clicks an undo button
  // showing a stale snapshot list — making the click look "ineffective"
  // until the next refresh.
  onEditsChanged: async (sid: number) => {
    await loadEdits(sid)
  },
})
...
const { recentEdits, lastUndone, showUndoToast, lastUndoError, loadEdits, undoLast } = useUndoHistory()
...
const thinkingExpanded = ref<Record<string | number, boolean>>({})
...
function saveScrollPos() {
  ...
  scrollRafId = requestAnimationFrame(() => {
    ...
    updateAtBottom()
    if (!navScrolling.value) debouncedSyncNav()
  })
}

// Preserve scroll position when loading older messages (prepend shifts content)
async function handleLoadMore() {
  if (!messagesEl.value) { await loadMoreMessages(); return }
...
function isThinkingExpanded(id: string | number): boolean {
  return thinkingExpanded.value[id] !== false
}

function toggleThinking(id: string | number) {
...
const EMPTY_STREAMING = Object.freeze({
  text: '', thinking: '', done: true, started: false, toolCallCount: 0, toolEvents: [] as never[]
})

const currentStreaming = computed(() => {
  const entry = sessions.value.get(streamKey.value)
  if (!entry?.state) return EMPTY_STREAMING
  return entry.state
})
...
const streamingSegments = computed(() => {
  ...
  // Diagnostic: detect stale toolEvents (tool_start not synced to state)
  if (import.meta.env.DEV && events.length > 0) {
    ...
    if (starts !== ends) {
      console.warn('[streamingSegments] toolEvents out of sync:', { starts, ends, textLen: fullText.length })
    }
  }
...
const showLoading = computed(() => {
  ...
  return loading.value && cs && !cs.done && !cs.started && (!cs.toolEvents || cs.toolEvents.length === 0)
})

// Markdown render cache — renderMarkdown with hljs is expensive
const mdCache = new Map<string, string>()
const MAX_MD_CACHE = 200 // Reduced from 600 to save memory
...
function formatContent(text: string, streaming?: boolean): string {
  ...
  if (mdCache.size >= MAX_MD_CACHE) {
    ...
    const keysToDelete = [...mdCache.keys()].slice(0, mdCache.size - MAX_MD_CACHE + 1)
...
const attachmentsCache = new WeakMap<object, AttInfo[] | null>()
function parsedAttachments(msg: { attachments?: string }): AttInfo[] | null {
  const cached = attachmentsCache.get(msg)
  if (cached !== undefined) return cached
  if (!msg.attachments) { attachmentsCache.set(msg, null); return null }
  try {
    const arr = typeof msg.attachments === 'string' ? JSON.parse(msg.attachments) : msg.attachments
    if (!Array.isArray(arr) || arr.length === 0) { attachmentsCache.set(msg, null); return null }
    attachmentsCache.set(msg, arr)
    return arr
  } catch { attachmentsCache.set(msg, null); return null }
}
...
interface ToolCardInfo { name: string; args?: string; result?: string; tool_id?: string }

const toolCardInfoCache = new WeakMap<object, ToolCardInfo | null>()
function getToolCardInfo(msg: { role: string; content?: string; tool_calls?: string }): ToolCardInfo | null {
  const cached = toolCardInfoCache.get(msg)
  if (cached !== undefined) return cached
  if (!msg.tool_calls) { toolCardInfoCache.set(msg, null); return null }
  try {
    const calls = JSON.parse(msg.tool_calls)
    if (!Array.isArray(calls) || calls.length === 0) { toolCardInfoCache.set(msg, null); return null }
    const tc = calls[0]
    const name = tc.function?.name || 'tool'
    if (name === 'todo_write') { toolCardInfoCache.set(msg, null); return null }
    const result: ToolCardInfo = {
      name,
      args: tc.function?.arguments || undefined,
      result: msg.content || undefined,
      tool_id: tc.id || undefined,
    }
    toolCardInfoCache.set(msg, result)
    return result
  } catch { toolCardInfoCache.set(msg, null); return null }
}
...
// Uses full DB user message list, not paginated displayMessages.
...
// Full list of user message IDs from DB (not paginated)
const allUserMsgIds = ref<number[]>([])
// navCurrent tracks the nav cursor index into allUserMsgIds
const navCurrent = ref(0)

async function refreshUserMsgIds() {
  const sid = sessionId.value
  if (!sid) { allUserMsgIds.value = []; return }
  const ids = await db.getUserMessageIndices(sid)
  allUserMsgIds.value = ids
  console.log('[nav] refreshUserMsgIds session=%d count=%d ids=%o', sid, ids.length, ids)
}

/** Find the display message for a given user message DB ID. */
function findDisplayMsg(msgId: number) {
  return displayMessages.value.find(m => (m as any).dbId === msgId || m.id === msgId)
}

/** Ensure the page containing msgId is loaded, then return the display message. */
async function ensureLoaded(msgId: number) {
  let found = findDisplayMsg(msgId)
  if (found) return found
  // Message not loaded — load pages until it appears
  while (hasMoreOlder.value && !loadingMore.value) {
    await handleLoadMore()
    found = findDisplayMsg(msgId)
    if (found) return found
  }
  return null
}
...
function syncNavFromScroll() {
  const ids = allUserMsgIds.value
  if (ids.length === 0 || !messagesEl.value) return
  const top = messagesEl.value.scrollTop
  for (let j = ids.length - 1; j >= 0; j--) {
    const dispMsg = findDisplayMsg(ids[j])
    if (!dispMsg) continue
    const el = messagesEl.value.querySelector(`[data-msg-id="${dispMsg.id}"]`) as HTMLElement | null
...
// Debounced syncNavFromScroll — avoids DOM queries on every scroll event
let syncNavTimer: ReturnType<typeof setTimeout> | null = null
function debouncedSyncNav() {
  if (syncNavTimer) clearTimeout(syncNavTimer)
  syncNavTimer = setTimeout(syncNavFromScroll, 100)
}

async function navToUserMsg(direction: 1 | -1) {
  const ids = allUserMsgIds.value
  if (ids.length === 0) return
  const target = Math.max(0, Math.min(ids.length - 1, navCurrent.value + direction))
  navCurrent.value = target
  const msgId = ids[target]

  // Find the message in displayMessages (loads older pages if needed)
  const dispMsg = await ensureLoaded(msgId)
  if (!dispMsg || !messagesEl.value) return

  // Wait for DOM update after potential pagination
  await nextTick()
...
const userMsgNav = computed(() => ({
  ...
  count: allUserMsgIds.value.length,
}))
    ...
    try {
      ...
      if (text === '/compact') {
        ...
        if (!workspace.value) {
          ...
          const content = await invoke<string>('read_file', { path: `${workspace.value}/minimax.md` }).catch(() => '')
          if (content) {
...
-
`
            await invoke('write_file', { path: `${workspace.value}/minimax.md`, content: template })
...
const workspace = ref('')
const workspaceInputEl = ref<HTMLInputElement>()
...
async function loadWorkspace() {
  ...
  try {
    const gcWorkspace = await invoke<string | null>('get_group_chat_workspace', { groupChatId: props.groupChatId })
    workspace.value = gcWorkspace || await invoke<string>('get_workspace').catch(() => '') || ''
  } catch { workspace.value = '' }
}

// Debounce timer for auto-saving workspace
let workspaceSaveTimer: ReturnType<typeof setTimeout> | null = null

function onWorkspaceInput(e: Event) {
  const target = e.target as HTMLInputElement
  workspace.value = target.value
...
async function saveWorkspaceEdit() {
  ...
  const trimmed = workspaceInputEl.value?.value.trim() || ''
  if (trimmed && trimmed !== workspace.value && props.groupChatId) {
    workspace.value = trimmed
  }
  if (props.groupChatId) {
    try {
      await invoke('set_group_chat_workspace', { groupChatId: props.groupChatId, workspace: workspace.value })
    } catch (e) {
...
function cancelWorkspaceEdit() {
  ...
  // Restore the displayed value to the saved workspace
  if (workspaceInputEl.value) {
    workspaceInputEl.value.value = workspace.value
  }
}

async function pickWorkspaceFolder() {
  const selected = await open({ directory: true, multiple: false, title: '选择工作目录' })
  if (selected && props.groupChatId) {
    workspace.value = selected as string
    try {
      await invoke('set_group_chat_workspace', { groupChatId: props.groupChatId, workspace: selected as string })
    } catch (e) {
      console.error('Failed to save workspace:', e)
    }
  }
...
// Load edits, bookmarks, and user message nav index when session changes
watch(sessionId, async (sid) => {
  if (sid) {
    loadEdits(sid)
    loadBookmarks(sid)
    await refreshUserMsgIds()
    loadWorkspace()
    await nextTick()
    // Sync nav to current scroll position after DOM renders
    syncNavFromScroll()
  } else {
    // Session cleared (new temp chat) — reset all local state
    allUserMsgIds.value = []
    navCurrent.value = 0
    recentEdits.value = []
    bookmarks.value = []
...
// Refresh user message nav index + edits when messages change (covers stream complete, rewind, etc.)
watch(() => messages.value.length, async () => {
  if (sessionId.value) {
    await refreshUserMsgIds()
    loadEdits(sessionId.value)
    await nextTick()
    syncNavFromScroll()
  }
})
...
// Streaming auto-scroll: let browser's native scroll anchoring handle it.
// Only force-scroll if user is very close to bottom (< 200px).
...
// Keep navCurrent in bounds when user message IDs change
watch(allUserMsgIds, (ids) => {
  if (ids.length === 0) {
    navCurrent.value = 0
  } else if (navCurrent.value >= ids.length) {
    navCurrent.value = ids.length - 1
  }
})
